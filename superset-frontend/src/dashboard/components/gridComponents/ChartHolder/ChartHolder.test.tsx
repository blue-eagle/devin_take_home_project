...
test('ChartHolder should render empty state', async () => {
  renderWrapper();
...
test('ChartHolder should render anchor link when not editing', async () => {
  const store = createMockStore();
  const { rerender } = renderWrapper(store, { editMode: false });

  expect(screen.getByTestId('dashboard-component-chart-holder')).toBeVisible();
...
test('ChartHolder should highlight when path matches', async () => {
  const store = createMockStore({
    dashboardState: {
...
test('ChartHolder should calculate the default widthMultiple', async () => {
...
test('ChartHolder should set the resizable width to auto when parent component type is column', async () => {
  renderWrapper(createMockStore(), {
    editMode: true,
    parentComponent: {
...
test("ChartHolder should override the widthMultiple if there's a column in the parent chain whose width is less than the chart", async () => {
...
test('ChartHolder should calculate the chartWidth', async () => {
...
test('ChartHolder should calculate the chartWidth on full screen mode', async () => {
  const widthMultiple = 7;
  const columnWidth = 250;
  renderWrapper(createMockStore(), {
    component: {
...
test('ChartHolder should calculate the chartHeight', async () => {
...
test('ChartHolder should calculate the chartHeight on full screen mode', async () => {
  const heightMultiple = 12;
  renderWrapper(createMockStore(), {
    component: {
      ...defaultProps.component,
...
test('ChartHolder should call deleteComponent when deleted', async () => {
  ...
  fireEvent.click(
    screen.getByTestId('dashboard-delete-component-button').firstElementChild!,
  );
