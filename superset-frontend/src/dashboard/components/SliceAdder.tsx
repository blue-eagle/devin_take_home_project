...
function SliceAdder({
  ...
  const slicesRequestRef = useRef<AbortController | Promise<void> | undefined>();
  ...
  const prevSelectedSliceIds = useRef(selectedSliceIds);

  useEffect(() => {
    if (lastUpdated !== prevLastUpdated.current) {
      setFilteredSlices(
        getFilteredSortedSlices(slices, searchTerm, sortBy, showOnlyMyCharts, userId),
      );
      prevLastUpdated.current = lastUpdated;
    }
  }, [lastUpdated, slices, searchTerm, sortBy, showOnlyMyCharts, userId]);

  useEffect(() => {
    if (prevSelectedSliceIds.current !== selectedSliceIds) {
      setSelectedSliceIdsSet(new Set(selectedSliceIds));
      prevSelectedSliceIds.current = selectedSliceIds;
    }
  }, [selectedSliceIds]);

  useEffect(
    () => () => {
      const selectedSlicesLocal = pickBy(slices, (value: Slice) =>
        selectedSliceIdsSet.has(value.slice_id),
      );
      updateSlices(selectedSlicesLocal);
      if (slicesRequestRef.current instanceof AbortController) {
        slicesRequestRef.current.abort();
      }
    },
    [slices, selectedSliceIdsSet, updateSlices],
  );
  ...
  const handleChange = useMemo(
    () =>
      debounce((value: string) => {
        setSearchTerm(value);
        setFilteredSlices(
          getFilteredSortedSlices(slices, value, sortBy, showOnlyMyCharts, userId),
        );
        slicesRequestRef.current = fetchSlices(
          showOnlyMyCharts ? userId : undefined,
          value,
    ...
    [slices, sortBy, showOnlyMyCharts, userId, fetchSlices],
  );
  ...
  const handleSelect = useCallback(
    (newSortBy: keyof Slice) => {
      setSortBy(newSortBy);
      setFilteredSlices(
        getFilteredSortedSlices(slices, searchTerm, newSortBy, showOnlyMyCharts, userId),
      );
      slicesRequestRef.current = fetchSlices(
        showOnlyMyCharts ? userId : undefined,
        searchTerm,
    ...
    [slices, searchTerm, showOnlyMyCharts, userId, fetchSlices],
  );
  ...
  const onShowOnlyMyCharts = useCallback(
    (checked: boolean) => {
      ...
      setFilteredSlices(
        getFilteredSortedSlices(slices, searchTerm, sortBy, checked, userId),
      );
      setItem(LocalStorageKeys.DashboardEditorShowOnlyMyCharts, checked);
    },
    [fetchSlices, searchTerm, sortBy, slices, userId],
  );
    ...
    >
      ...
      <div
        css={thm => css`
          ...
          align-items: center;
          gap: ${thm.sizeUnit}px;
          padding: 0 ${thm.sizeUnit * 3}px ${thm.sizeUnit * 4}px
            ${thm.sizeUnit * 3}px;
        `}
      >
        <Checkbox
