...
test('dashboardState actions saveDashboardRequest should dispatch UPDATE_COMPONENTS_PARENTS_LIST action', () => {
  ...
  expect(dispatch.mock.calls[0][0].type).toBe(UPDATE_COMPONENTS_PARENTS_LIST);
  expect(dispatch.mock.calls[1][0].type).toBe(SAVE_DASHBOARD_STARTED);
});

test('dashboardState actions saveDashboardRequest should post dashboard data with updated redux state', () => {
  const { getState, dispatch } = setup({
    dashboardState: { hasUnsavedChanges: false },
  });
  ...
  expect(parsedJsonMetadata.positions[DASHBOARD_GRID_ID].parents).toStrictEqual(
    mockParentsList,
  );
});
...
test('dashboardState actions saveDashboardRequest FeatureFlag.CONFIRM_DASHBOARD_DIFF dispatches SET_OVERRIDE_CONFIRM when an inspect value has diff', async () => {
  const id = 192;
  const { getState, dispatch } = setup();
  const thunk = saveDashboardRequest(newDashboardData, id, SAVE_TYPE_OVERWRITE);
  thunk(dispatch, getState);
  ...
  expect(dispatch.mock.calls[2][0].overwriteConfirmMetadata.dashboardId).toBe(
    id,
  );
});

test('dashboardState actions saveDashboardRequest FeatureFlag.CONFIRM_DASHBOARD_DIFF should post dashboard data with after confirm the overwrite values', async () => {
  const id = 192;
  const { getState, dispatch } = setup();
...
test('dashboardState actions saveDashboardRequest should navigate to the new dashboard after Save As', async () => {
...
test('dashboardState actions fetchCharts returns a Promise that resolves after all refreshes', async () => {
...
test('dashboardState actions fetchCharts resolves for staggered refreshes', async () => {
  jest.useFakeTimers();
  (refreshChart as jest.Mock).mockClear();
  const { getState } = setup({
    dashboardInfo: {
...
test('dashboardState actions fetchCharts rejects for staggered refreshes when any chart refresh fails', async () => {
  jest.useFakeTimers();
  (refreshChart as jest.Mock).mockClear();
  ...
  jest.useRealTimers();
  (refreshChart as jest.Mock).mockImplementation(() => () => Promise.resolve());
});

test('dashboardState actions onRefresh dispatches success and filters refresh by default', async () => {
  const { getState } = setup({
    dashboardInfo: {
      metadata: {},
      common: { conf: { SUPERSET_WEBSERVER_TIMEOUT: 60 } },
...
test('dashboardState actions onRefresh skips filter refresh when requested', async () => {
  const { getState } = setup({
    dashboardInfo: {
      metadata: {},
      common: { conf: { SUPERSET_WEBSERVER_TIMEOUT: 60 } },
...
test('dashboardState actions onRefresh skips ON_REFRESH and filters refresh for lazy-loaded tabs', async () => {
  const { getState } = setup({
    dashboardInfo: {
      metadata: {},
      common: { conf: { SUPERSET_WEBSERVER_TIMEOUT: 60 } },
