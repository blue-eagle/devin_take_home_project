...
test('getUpToDateQuery should return the up to date query editor state', () => {
...
test('async actions saveQuery posts the correct query object', () => {
  const store = mockStore(initialState);
  return store.dispatch(actions.saveQuery(query, queryId)).then(() => {
...
test('async actions saveQuery calls 3 dispatch actions', () => {
  expect.assertions(1);

  return makeSaveQueryRequest().then(() => {
    expect(dispatch.mock.calls.length).toBe(2);
  });
});

test('async actions saveQuery calls QUERY_EDITOR_SAVED after making a request', () => {
  expect.assertions(1);
...
test('async actions saveQuery onSave calls QUERY_EDITOR_SAVED and QUERY_EDITOR_SET_TITLE', () => {
  expect.assertions(1);
...
test('async actions formatQuery posts to the correct url', async () => {
  const store = mockStore(initialState);
  store.dispatch(actions.formatQuery(query as unknown as QueryEditor));
  await waitFor(() =>
    expect(fetchMock.callHistory.calls(formatQueryEndpoint)).toHaveLength(1),
  );
  expect(store.getActions()[0].type).toBe(actions.QUERY_EDITOR_SET_SQL);
  expect(store.getActions()[0].sql).toBe(expectedSql);
});

test('async actions formatQuery sends only sql in request body when no dbId or templateParams', async () => {
...
test('async actions formatQuery includes database_id in request when dbId is provided', async () => {
...
test('async actions formatQuery includes template_params as string when provided as string', async () => {
...
test('async actions formatQuery stringifies template_params when provided as object', async () => {
...
test('async actions formatQuery dispatches QUERY_EDITOR_SET_SQL with formatted result', async () => {
...
test('async actions formatQuery uses up-to-date query editor state from store', async () => {
...
const makeFetchQueryResultsRequest = () => {
  const store = mockStore(initialState);
  const request = actions.fetchQueryResults(query);
  return request(dispatch, store.getState, undefined);
};

test('async actions fetchQueryResults makes the fetch request', () => {
  expect.assertions(1);

  return makeFetchQueryResultsRequest().then(() => {
    expect(fetchMock.callHistory.calls(fetchQueryEndpoint)).toHaveLength(1);
  });
});

test('async actions fetchQueryResults calls requestQueryResults', () => {
  expect.assertions(1);

  return makeFetchQueryResultsRequest().then(() => {
    expect(dispatch.mock.calls[0][0].type).toBe(actions.REQUEST_QUERY_RESULTS);
  });
});

/* oxlint-disable-next-line jest/no-disabled-tests */
test.skip('parses large number result without losing precision', () =>
  makeFetchQueryResultsRequest().then(() => {
    expect(fetchMock.callHistory.calls(fetchQueryEndpoint)).toHaveLength(1);
    expect(dispatch.mock.calls.length).toBe(2);
    expect(
...
test('async actions fetchQueryResults calls querySuccess on fetch success', () => {
  expect.assertions(1);
...
test('async actions fetchQueryResults calls queryFailed on fetch error', () => {
  expect.assertions(1);
...
const makeRunQueryRequest = () => {
  const request = actions.runQuery(query);
  return request(dispatch, () => typedInitialState, undefined);
};

test('async actions runQuery without query params makes the fetch request', () => {
  expect.assertions(1);

  return makeRunQueryRequest().then(() => {
    expect(fetchMock.callHistory.calls(runQueryEndpoint)).toHaveLength(1);
  });
});

test('async actions runQuery without query params calls startQuery', () => {
  expect.assertions(1);
...
test('async actions runQuery without query params parses large number result without losing precision', () =>
  makeRunQueryRequest().then(() => {
    expect(fetchMock.callHistory.calls(runQueryEndpoint)).toHaveLength(1);
    expect(dispatch.mock.calls.length).toBe(2);
    expect(
      dispatch.mock.calls[1][
...
test('async actions runQuery without query params calls querySuccess on fetch success', () => {
  expect.assertions(1);
...
test('async actions runQuery without query params calls queryFailed on fetch error and logs the error details', () => {
  expect.assertions(2);
...
const makeRunQueryWithParamsRequest = () => {
  const request = actions.runQuery(query);
  return request(dispatch, () => typedInitialState, undefined);
};

test('async actions runQuery with query params makes the fetch request', async () => {
  const runQueryEndpointWithParams = 'glob:*/api/v1/sqllab/execute/?foo=bar';
  fetchMock.post(runQueryEndpointWithParams, `{ "data": ${mockBigNumber} }`);
  await makeRunQueryWithParamsRequest().then(() => {
    expect(
...
test('async actions reRunQuery creates new query with a new id', () => {
  const id = 'id';
  const state = {
    sqlLab: {
      tabHistory: [id],
...
const makeStopQueryRequest = () => {
  const request = actions.postStopQuery(baseQuery);
  return request(dispatch, () => typedInitialState, undefined);
};

test('async actions postStopQuery makes the fetch request', () => {
  expect.assertions(1);

  return makeStopQueryRequest().then(() => {
    expect(fetchMock.callHistory.calls(stopQueryEndpoint)).toHaveLength(1);
  });
});

test('async actions postStopQuery calls stopQuery', () => {
  expect.assertions(1);
...
test('async actions postStopQuery sends the correct data', () => {
  expect.assertions(1);
...
test('async actions cloneQueryToNewTab creates new query editor', () => {
  expect.assertions(1);
...
const popSavedQueryStore = mockStore({});
...
const makePopSavedQueryRequest = (id: string | number) => {
  const request = actions.popSavedQuery(String(id));
  const { dispatch } = popSavedQueryStore;
...
beforeEach(() => {
  ...
  popSavedQueryStore.clearActions();
});
...
test('async actions popSavedQuery calls API endpint with correct params', async () => {
  supersetClientGetSpy.mockResolvedValue({
    json: { result: mockSavedQueryApiResponse },
  } as any);

  await makePopSavedQueryRequest(123);
...
test('async actions popSavedQuery dispatches addQueryEditor with correct params on successful API call', async () => {
  supersetClientGetSpy.mockResolvedValue({
    json: { result: mockSavedQueryApiResponse },
  } as any);
  ...
  await makePopSavedQueryRequest(1);

  const addQueryEditorAction = popSavedQueryStore
    .getActions()
...
test('async actions popSavedQuery should dispatch addDangerToast on API error', async () => {
  supersetClientGetSpy.mockResolvedValue(new Error() as any);

  await makePopSavedQueryRequest(1);

  const addToastAction = popSavedQueryStore
    .getActions()
...
test('async actions addQueryEditor creates new query editor', () => {
  expect.assertions(1);
...
test('async actions addQueryEditor addNewQueryEditor creates new query editor with new tab name', () => {
  const store = mockStore({
    ...initialState,
    sqlLab: {
      ...initialState.sqlLab,
...
test('async actions addQueryEditor addNewQueryEditor creates a new query editor from the saved state in the empty tab', () => {
...
test('async actions set current query editor', () => {
  expect.assertions(1);
...
test('async actions swithQueryEditor switch to the next tab editor', () => {
  const store = mockStore(initialState);
  const expectedActions = [
    {
      type: actions.SET_ACTIVE_QUERY_EDITOR,
...
test('async actions swithQueryEditor switch to the first tab editor once it reaches the rightmost tab', () => {
  const store = mockStore({
    ...initialState,
    sqlLab: {
      ...initialState.sqlLab,
...
test('async actions swithQueryEditor switch to the previous tab editor', () => {
  const store = mockStore({
    ...initialState,
    sqlLab: {
      ...initialState.sqlLab,
...
test('async actions swithQueryEditor switch to the last tab editor once it reaches the leftmost tab', () => {
  const store = mockStore({
    ...initialState,
    sqlLab: {
      ...initialState.sqlLab,
...
test('async actions backend sync addQueryEditor creates the tab state in the local storage', () => {
  expect.assertions(2);
  ...
  expect(store.getActions()).toEqual(expectedActions);
  expect(fetchMock.callHistory.calls(updateTabStateEndpoint)).toHaveLength(0);
});

test('async actions backend sync removeQueryEditor updates the tab state in the backend', () => {
  expect.assertions(1);
...
test('async actions backend sync queryEditorSetDb updates the tab state in the backend', () => {
  expect.assertions(1);
...
test('async actions backend sync queryEditorSetCatalog updates the tab state in the backend', () => {
  expect.assertions(1);
...
test('async actions backend sync queryEditorSetSchema updates the tab state in the backend', () => {
  expect.assertions(1);
...
test('async actions backend sync queryEditorSetAutorun updates the tab state in the backend', () => {
  expect.assertions(1);
...
test('async actions backend sync queryEditorSetTitle updates the tab state in the backend', () => {
  expect.assertions(1);
...
test('async actions backend sync queryEditorSetAndSaveSql with backend persistence flag on updates the tab state in the backend', () => {
  expect.assertions(2);
  ...
  return request(store.dispatch, store.getState, undefined).then(() => {
    expect(store.getActions()).toEqual(expectedActions);
    expect(fetchMock.callHistory.calls(updateTabStateEndpoint)).toHaveLength(1);
  });
});

test('async actions backend sync queryEditorSetAndSaveSql with backend persistence flag off does not update the tab state in the backend', () => {
  isFeatureEnabledMock.mockImplementation(
    (feature: string) => !(feature === 'SQLLAB_BACKEND_PERSISTENCE'),
  );
...
test('async actions backend sync queryEditorSetQueryLimit updates the tab state in the backend', () => {
  expect.assertions(1);
  ...
  store.dispatch(actions.queryEditorSetQueryLimit(queryEditor, queryLimit));
  expect(store.getActions()).toEqual(expectedActions);
});

test('async actions backend sync queryEditorSetTemplateParams updates the tab state in the backend', () => {
  expect.assertions(1);
...
test('async actions backend sync addTable dispatches table state from unsaved change', () => {
  ...
  const request = actions.addTable(query, tableName, catalogName, schemaName);
  request(store.dispatch, store.getState, undefined);
...
test('async actions backend sync addTable uses tabViewId when available', () => {
  const tableName = 'table';
  const catalogName = null;
  const schemaName = 'schema';
  const expectedDbId = 473892;
...
test('async actions backend sync addTable falls back to id when tabViewId is not available', () => {
  const tableName = 'table';
  const catalogName = null;
  const schemaName = 'schema';
  const expectedDbId = 473892;
...
test('async actions backend sync syncTable updates the table schema state in the backend', () => {
  expect.assertions(4);
  ...
  const request = actions.syncTable(query as any, tableName as any, schemaName);
  return request(store.dispatch, store.getState, undefined).then(() => {
    expect(store.getActions().map(a => a.type)).toEqual(expectedActionTypes);
...
test('async actions backend sync runTablePreviewQuery updates and runs data preview query when configured', () => {
  expect.assertions(3);
...
test('async actions backend sync runTablePreviewQuery runs data preview query only', () => {
  const expectedActionTypes = [
    actions.START_QUERY, // runQuery (data preview)
    actions.QUERY_SUCCESS, // querySuccess
  ];
...
test('async actions backend sync expandTable updates the table schema state in the backend when initialized', () => {
  expect.assertions(2);
...
test('async actions backend sync expandTable does not call backend when table is not initialized', () => {
  expect.assertions(2);
...
test('async actions backend sync expandTable does not call backend when initialized is undefined', () => {
  expect.assertions(2);
...
test('async actions backend sync expandTable does not call backend when feature flag is off', () => {
  expect.assertions(2);
...
test('async actions backend sync collapseTable updates the table schema state in the backend when initialized', () => {
  expect.assertions(2);
...
test('async actions backend sync collapseTable does not call backend when table is not initialized', () => {
  expect.assertions(2);
...
test('async actions backend sync collapseTable does not call backend when initialized is undefined', () => {
  expect.assertions(2);
...
test('async actions backend sync collapseTable does not call backend when feature flag is off', () => {
  expect.assertions(2);
...
test('async actions backend sync removeTables updates the table schema state in the backend', () => {
  expect.assertions(2);
...
test('async actions backend sync removeTables deletes multiple tables and updates the table schema state in the backend', () => {
  expect.assertions(2);
...
test('async actions backend sync removeTables only updates the initialized table schema state in the backend', () => {
  expect.assertions(2);
...
test('async actions backend sync syncQueryEditor updates the tab state in the backend', () => {
  expect.assertions(3);
  ...
  return store.dispatch(actions.syncQueryEditor(oldQueryEditor)).then(() => {
    expect(store.getActions()).toEqual(expectedActions);
    expect(fetchMock.callHistory.calls(updateTabStateEndpoint)).toHaveLength(3);
