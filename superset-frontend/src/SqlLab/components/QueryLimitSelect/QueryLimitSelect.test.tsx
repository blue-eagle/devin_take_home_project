...
test('QueryLimitSelect renders current query limit size', () => {
  const queryLimit = 10;
...
test('QueryLimitSelect renders default query limit for initial queryEditor', () => {
  const { getByText } = setup({}, mockStore(initialState));
  expect(getByText(defaultQueryLimit)).toBeInTheDocument();
});

test('QueryLimitSelect renders queryLimit from unsavedQueryEditor', () => {
...
test('QueryLimitSelect renders dropdown select', async () => {
...
test('QueryLimitSelect renders dropdown select correctly when maxRow is less than 10', async () => {
  const { baseElement, getAllByRole, getByRole } = setup(
    { maxRow: 5 },
    mockStore(initialState),
  );
...
test('QueryLimitSelect renders dropdown select correctly when maxRow is a multiple of 10', async () => {
  const { baseElement, getAllByRole, getByRole } = setup(
    { maxRow: 10000 },
    mockStore(initialState),
  );
...
test('QueryLimitSelect dispatches QUERY_EDITOR_SET_QUERY_LIMIT action on dropdown menu click', async () => {
