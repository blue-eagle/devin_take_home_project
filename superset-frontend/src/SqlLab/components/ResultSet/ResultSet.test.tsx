...
test('ResultSet renders a Table', async () => {
...
test('ResultSet should render success query', async () => {
...
test('ResultSet should render empty results', async () => {
  const query = {
    ...queries[0],
    results: { data: [] },
  };
...
test('ResultSet should call reRunQuery if timed out', async () => {
  const query = {
    ...queries[0],
...
test('ResultSet should not call reRunQuery if no error', async () => {
  const query = queries[0];
  const store = mockStore({
    ...initialState,
    user,
...
test('ResultSet should render cached query', async () => {
...
test('ResultSet should render stopped query', async () => {
  await waitFor(() => {
    setup(
...
test('ResultSet should render a failed query with an errors object', async () => {
...
test('ResultSet should render a timeout error with a retrial button', async () => {
  await waitFor(() => {
    setup(
...
test('ResultSet renders if there is no limit in query.results but has queryLimit', async () => {
  const query = {
    ...queries[0],
  };
  await waitFor(() => {
...
test('ResultSet renders if there is a limit in query.results but not queryLimit', async () => {
...
test('ResultSet Async queries - renders "Fetch data preview" button when data preview has no results', () => {
...
test('ResultSet Async queries - renders "Refetch results" button when a query has no results', () => {
...
test('ResultSet Async queries - renders on the first call', () => {
  const query = {
    ...queries[0],
  };
  setup(
...
test('ResultSet should display a popup message when the CSV content is limited to the dropdown limit', async () => {
  const queryLimit = 2;
...
test('ResultSet should not allow download as CSV when user does not have permission to export data', async () => {
  const { queryByTestId } = setup(
    mockedProps,
    mockStore({
      ...initialState,
...
test('ResultSet should allow copy to clipboard when user has permission to export data', async () => {
  const { queryByTestId } = setup(
    mockedProps,
    mockStore({
      ...initialState,
...
test('ResultSet should not allow copy to clipboard when user does not have permission to export data', async () => {
  const { queryByTestId } = setup(
    mockedProps,
    mockStore({
      ...initialState,
...
test('ResultSet should include sqlEditorImmutableId in query object when fetching results', async () => {
...
test('ResultSet should use non-streaming export (href) when rows below threshold', async () => {
...
test('ResultSet should show CSV button with granular can_export_data permission when flag is ON', async () => {
...
test('ResultSet should disable CSV button when granular flag is ON and user lacks can_export_data', async () => {
  mockIsFeatureEnabled.mockImplementation(
    (flag: FeatureFlag) => flag === FeatureFlag.GranularExportControls,
  );
  const { queryByTestId } = setup(
...
test('ResultSet should show clipboard button with granular can_copy_clipboard permission when flag is ON', async () => {
  mockIsFeatureEnabled.mockImplementation(
    (flag: FeatureFlag) => flag === FeatureFlag.GranularExportControls,
  );
  const { queryByTestId } = setup(
...
test('ResultSet should disable clipboard button when granular flag is ON and user lacks can_copy_clipboard', async () => {
  mockIsFeatureEnabled.mockImplementation(
    (flag: FeatureFlag) => flag === FeatureFlag.GranularExportControls,
  );
  const { queryByTestId } = setup(
...
test('ResultSet should use legacy can_export_csv for both CSV and clipboard when granular flag is OFF', async () => {
...
test('ResultSet disabled CSV button should show permission tooltip when granular flag is ON', async () => {
  mockIsFeatureEnabled.mockImplementation(
    (flag: FeatureFlag) => flag === FeatureFlag.GranularExportControls,
  );
  const { getByTestId } = setup(
...
test('ResultSet disabled clipboard button should show permission tooltip when granular flag is ON', async () => {
  mockIsFeatureEnabled.mockImplementation(
    (flag: FeatureFlag) => flag === FeatureFlag.GranularExportControls,
  );
  const { getByTestId } = setup(
...
test('ResultSet renders contributed toolbar action in results slot', async () => {
  registerToolbarAction(
