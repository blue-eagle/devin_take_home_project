...
test('sqlLabReducer Query editors actions should add a query editor', () => {
  expect(newState.queryEditors).toHaveLength(
    initialState.queryEditors.length + 1,
  );
});

test('sqlLabReducer Query editors actions should merge the current unsaved changes when adding a query editor', () => {
  ...
  expect(newState.queryEditors[newState.queryEditors.length - 1].id).toEqual(
    'efgh',
  );
});

test('sqlLabReducer Query editors actions should remove a query editor', () => {
  expect(newState.queryEditors).toHaveLength(
    initialState.queryEditors.length + 1,
  );
  const action = {
  ...
  newState = sqlLabReducer(newState, action);
  expect(newState.queryEditors).toHaveLength(initialState.queryEditors.length);
});

test('sqlLabReducer Query editors actions should select the latest query editor when tabHistory is empty', () => {
...
test('sqlLabReducer Query editors actions should remove a query editor including unsaved changes', () => {
  expect(newState.queryEditors).toHaveLength(
    initialState.queryEditors.length + 1,
  );
  const setDbAction = {
...
test('sqlLabReducer Query editors actions should set q query editor active', () => {
  const expectedTitle = 'new updated title';
...
test('sqlLabReducer Query editors actions should not fail while setting DB', () => {
  const dbId = 9;
  const action = {
    type: actions.QUERY_EDITOR_SETDB,
    queryEditor: qe,
...
test('sqlLabReducer Query editors actions should not fail while setting schema', () => {
  const schema = 'foo';
  const action = {
...
test('sqlLabReducer Query editors actions should not fail while setting autorun', () => {
  const action = {
...
test('sqlLabReducer Query editors actions should not fail while setting title', () => {
...
test('sqlLabReducer Query editors actions should not fail while setting Sql', () => {
...
test('sqlLabReducer Query editors actions should not fail while setting queryLimit', () => {
...
test('sqlLabReducer Query editors actions should set selectedText', () => {
...
test('sqlLabReducer Query editors actions should not wiped out unsaved changes while delayed async call intercepted', () => {
...
test('sqlLabReducer Query editors actions should migrate query editor by new query editor id', () => {
...
test('sqlLabReducer Query editors actions should clear the destroyed query editors', () => {
...
test('sqlLabReducer Tables should add a table', () => {
...
test('sqlLabReducer Tables should merge the table attributes', () => {
  ...
  newState = sqlLabReducer(newState, action);
  expect(newState.tables).toHaveLength(1);
  expect((newState.tables[0] as unknown as Record<string, unknown>).extra).toBe(
    true,
  );
});

test('sqlLabReducer Tables should overwrite table ID be ignored when the existing table is already initialized', () => {
  const action = {
    type: actions.MERGE_TABLE,
    table: newTable,
  };
...
test('sqlLabReducer Tables should expand and collapse a table', () => {
...
test('sqlLabReducer Tables should remove a table', () => {
  const action = {
...
test('sqlLabReducer Tables should set activeSouthPaneTab when adding expanded table', () => {
  const expandedTable = {
    ...table,
...
test('sqlLabReducer Tables should not set activeSouthPaneTab when adding collapsed table', () => {
...
test('sqlLabReducer Tables should set activeSouthPaneTab when merging existing table with expanded=true', () => {
...
test('sqlLabReducer Tables should not set activeSouthPaneTab when merging existing table with expanded=false', () => {
...
test('sqlLabReducer Run Query should start a query', () => {
  const action = {
...
test('sqlLabReducer Run Query should stop the query', () => {
...
test('sqlLabReducer Run Query should remove a query', () => {
  const startQueryAction = {
    type: actions.START_QUERY,
    query,
  };
...
test('sqlLabReducer Run Query should refresh queries when polling returns new results', () => {
...
test('sqlLabReducer Run Query should skip refreshing queries when polling contains existing results', () => {
...
test('sqlLabReducer Run Query should refresh queries when polling returns empty', () => {
...
test('sqlLabReducer Run Query should set state to fetching when sync query succeeds without results', () => {
  const syncQuery = {
    ...query,
    id: 'sync-query',
...
test('sqlLabReducer CLEAR_INACTIVE_QUERIES updates queries that have already been completed', () => {
  newState = sqlLabReducer(
    {
      ...newState,
      queries: {
