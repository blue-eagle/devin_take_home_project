...
test('sparklineDataUtils parseTimeRatio should parse string values', () => {
  expect(parseTimeRatio('5')).toBe(5);
  expect(parseTimeRatio('10')).toBe(10);
});

test('sparklineDataUtils parseTimeRatio should pass through number values', () => {
  expect(parseTimeRatio(5)).toBe(5);
  expect(parseTimeRatio(10)).toBe(10);
});

test('sparklineDataUtils transformTimeRatioData should calculate ratios correctly', () => {
...
test('sparklineDataUtils transformTimeRatioData should handle zero values', () => {
...
test('sparklineDataUtils transformRegularData should map values directly', () => {
  const result = transformRegularData(mockEntries, 'sales');

  expect(result).toEqual([100, 200, 300, 400]);
});

test('sparklineDataUtils transformSparklineData should use time ratio when configured', () => {
...
test('sparklineDataUtils transformSparklineData should use regular data when no time ratio', () => {
...
test('sparklineDataUtils transformSparklineData should handle string time ratio', () => {
...
test('sparklineDataUtils parseSparklineDimensions should use provided values', () => {
...
test('sparklineDataUtils parseSparklineDimensions should use default values', () => {
  const column = { key: 'test' };
  const result = parseSparklineDimensions(column);
...
test('sparklineDataUtils validateYAxisBounds should return valid bounds', () => {
  const bounds = [0, 100];
...
test('sparklineDataUtils validateYAxisBounds should return default for invalid bounds', () => {
