...
test('Select buildQuery should build a default query', () => {
...
test('Select buildQuery should sort descending by metric', () => {
...
test('Select buildQuery should sort ascending by metric', () => {
  const queryContext = buildQuery({
    ...formData,
    sortMetric: 'my_metric',
    sortAscending: true,
...
test('Select buildQuery should sort ascending by column', () => {
  const queryContext = buildQuery({
    ...formData,
    sortAscending: true,
  });
...
test('Select buildQuery should sort descending by column', () => {
  const queryContext = buildQuery({
    ...formData,
    sortAscending: false,
  });
...
test('Select buildQuery should add text search parameter for string to query filter', () => {
  const queryContext = buildQuery(formData, {
    ownState: {
      search: 'abc',
...
test('Select buildQuery should add text search parameter for numeric to query filter', () => {
  const queryContext = buildQuery(formData, {
    ownState: {
      search: '123',
  ...
  expect(queryContext.queries.length).toEqual(1);
  const [query] = queryContext.queries;
  expect(query.filters).toEqual([{ col: 'my_col', op: 'ILIKE', val: '%123%' }]);
});
