...
export default function FixedOrMetricControl({
  ...
  const initialType = (propValue?.type ??
    defaultProp?.type ??
    controlTypes.fixed) as 'fix' | 'metric';
  const rawValue = propValue?.value ?? defaultProp?.value ?? '100';
  ...
  const fixedValueRef = useRef(fixedValue);
  const metricValueRef = useRef(metricValue);

  useEffect(() => {
    typeRef.current = type;
    fixedValueRef.current = fixedValue;
    metricValueRef.current = metricValue;
  }, [type, fixedValue, metricValue]);

  const fireOnChange = useCallback(() => {
    onChange({
      type: typeRef.current,
      value:
        typeRef.current === controlTypes.fixed
          ? fixedValueRef.current
          : (metricValueRef.current ?? undefined),
    });
  }, [onChange]);
  ...
  const setFixedValue = useCallback(
    (newFixedValue: string | number) => {
      ...
      fixedValueRef.current = newFixedValue;
      fireOnChange();
    },
    [fireOnChange],
  );
  ...
  const setMetric = useCallback(
    (newMetricValue: MetricValue | null) => {
      ...
      metricValueRef.current = newMetricValue;
      fireOnChange();
    },
    [fireOnChange],
  );

  const value = propValue ?? defaultProp;
  const currentType = value?.type ?? controlTypes.fixed;
  const columns = datasource ? datasource.columns : null;
  const metrics = datasource ? datasource.metrics : null;

  return (
    <div>
      <ControlHeader {...restProps} onChange={onChange} value={propValue} />
      <Collapse
        ghost
        items={[
          {
            ...
            children: (
              <div className="well">
                <PopoverSection
                  title={t('Fixed')}
                  isSelected={currentType === controlTypes.fixed}
                  onSelect={() => {
                ...
                <PopoverSection
                  ...
                  isSelected={currentType === controlTypes.metric}
                  onSelect={() => {
