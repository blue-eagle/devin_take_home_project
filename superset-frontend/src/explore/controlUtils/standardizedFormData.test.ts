...
const sourceMockStore = {
  ...
  controls: Object.fromEntries(
    Object.entries(sourceMockFormData).map(([key, value]) => [key, { value }]),
  ),
  datasource: {
    type: 'table',
    columns: [],
...
test('should collect control values and create SFD should avoid to overlap', () => {
...
test('should collect control values and create SFD should collect all sharedControls', () => {
...
test('should collect control values and create SFD should transform all publicControls and sharedControls', () => {
  ...
  expect(formData.columns).toEqual(['c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7']);
  expect(formData.metrics).toEqual(['m1', 'm2', 'm3', 'm4', 'm5', 'm6', 'm7']);
});

test('should collect control values and create SFD should inherit standardizedFormData and memorizedFormData is LIFO', () => {
...
test('should transform form_data between table and bigNumberTotal get and has', () => {
...
test('should transform form_data between table and bigNumberTotal transform', () => {
  // table -> bigNumberTotal
  const sfd = new StandardizedFormData(tableVizFormData);
...
test('should transform form_data between table and bigNumberTotal preserves dashboardId when transforming between viz types', () => {
...
test('should transform form_data between table and bigNumberTotal handles missing dashboardId gracefully', () => {
...
test('should transform form_data between table and bigNumberTotal handles null dashboardId', () => {
...
test('initial SFD between different datasource initial SFD between different datasource', () => {
  const sfd = new StandardizedFormData(tableVizFormData);
  // table -> big number
  const { formData: bntFormData, controlsState: bntControlsState } =
    sfd.transform(VizType.BigNumberTotal, tableVizStore);
  ...
  expect(
    newDatasourceSFD.serialize().memorizedFormData.map(([vizType]) => vizType),
