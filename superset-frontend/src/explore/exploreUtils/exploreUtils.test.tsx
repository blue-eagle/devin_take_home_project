...
test('exploreUtils getExploreUrl generates proper base url', () => {
...
test('exploreUtils getExploreUrl generates proper json url', () => {
  const url = getExploreUrl({
    formData,
    endpointType: 'json',
    force: false,
...
test('exploreUtils getExploreUrl generates proper json forced url', () => {
  const url = getExploreUrl({
    formData,
    endpointType: 'json',
    force: true,
...
test('exploreUtils getExploreUrl generates proper csv URL', () => {
  const url = getExploreUrl({
    formData,
    endpointType: 'csv',
    force: false,
...
test('exploreUtils getExploreUrl generates proper standalone URL', () => {
  const url = getExploreUrl({
    formData,
...
test('exploreUtils getExploreUrl preserves main URLs params', () => {
  const url = getExploreUrl({
    formData,
    endpointType: 'json',
    force: false,
...
test('exploreUtils getExploreUrl generate proper save slice url', () => {
  const url = getExploreUrl({
    formData,
    endpointType: 'json',
    force: false,
...
test('exploreUtils domain sharding generate url to different domains', () => {
...
test('exploreUtils domain sharding not generate url to different domains without flag', () => {
...
test('exploreUtils buildV1ChartDataPayload generate valid request payload despite no registered buildQuery', async () => {
...
test('exploreUtils getQuerySettings returns true for legacy viz', () => {
...
test('exploreUtils getQuerySettings returns false for v1 viz', () => {
  const [useLegacyApi, parseMethod] = getQuerySettings({
    ...formData,
    viz_type: 'my_v1_viz',
  });
...
test('exploreUtils getQuerySettings returns false for formData with unregistered viz_type', () => {
  const [useLegacyApi, parseMethod] = getQuerySettings({
    ...formData,
...
test('exploreUtils getQuerySettings returns false for formData without viz_type', () => {
  const [useLegacyApi, parseMethod] = getQuerySettings(formData);
  expect(useLegacyApi).toBe(false);
  expect(parseMethod).toBe('json-bigint');
});

test('exploreUtils getSimpleSQLExpression returns empty string when subject is undefined', () => {
  expect(getSimpleSQLExpression(undefined, '=', 10)).toBe('');
  expect(getSimpleSQLExpression()).toBe('');
});

test("exploreUtils getSimpleSQLExpression returns subject when it's provided and operator is undefined", () => {
  expect(getSimpleSQLExpression('col', undefined, 10)).toBe('col');
  expect(getSimpleSQLExpression('col')).toBe('col');
});

test("exploreUtils getSimpleSQLExpression returns subject and operator when they're provided and comparator is undefined", () => {
  expect(getSimpleSQLExpression('col', '=')).toBe('col =');
  expect(getSimpleSQLExpression('col', 'IN')).toBe('col IN');
  expect(getSimpleSQLExpression('col', 'IN', [])).toBe('col IN');
});

test('exploreUtils getSimpleSQLExpression returns full expression when subject, operator and comparator are provided', () => {
...
test('exploreUtils .exploreChart() postForm', () => {
