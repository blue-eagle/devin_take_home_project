...
test('useTables hook returns api response mapping json options', async () => {
...
test('useTables hook skips the deprecated schema option', async () => {
  const expectDbId = 'db1';
...
test('useTables hook returns hasMore when total is larger than result size', async () => {
  const expectDbId = 'db1';
...
test('useTables hook returns cached data without api request', async () => {
  const expectDbId = 'db1';
  const expectedSchema = 'schema1';
  const tableApiRoute = `glob:*/api/v1/database/${expectDbId}/tables/?q=*`;
  fetchMock.get(tableApiRoute, fakeApiResult);
...
test('useTables hook fetches tables without schema when supportsSchemas is false', async () => {
  const expectDbId = 'db1';
  const tableApiRoute = `glob:*/api/v1/database/${expectDbId}/tables/?q=*`;
  fetchMock.get(tableApiRoute, fakeApiResult);
  fetchMock.get(`glob:*/api/v1/database/${expectDbId}/catalogs/*`, {
...
test('useTables hook returns refreshed data after expires', async () => {
  const expectDbId = 'db1';
  const expectedSchema = 'schema1';
  const tableApiRoute = `glob:*/api/v1/database/${expectDbId}/tables/?q=*`;
  fetchMock.get(
  ...
  await waitFor(() => expect(result.current.data).toEqual(expectedHasMoreData));
