...
test('utils/common optionFromValue converts values as expected', () => {
...
test('utils/common prepareCopyToClipboardTabularData converts empty array', () => {
  const data: TabularDataRow[] = [];
  const columns: string[] = [];
  expect(prepareCopyToClipboardTabularData(data, columns)).toEqual('');
});

test('utils/common prepareCopyToClipboardTabularData converts non empty array', () => {
  const data: TabularDataRow[] = [
...
test('utils/common prepareCopyToClipboardTabularData includes 0 values and handle column objects', () => {
  const data: TabularDataRow[] = [
    { column1: 0, column2: 0 },
...
test('utils/common applyFormattingToTabularData does not mutate empty array', () => {
  const data: TabularDataRow[] = [];
  expect(applyFormattingToTabularData(data, [])).toEqual(data);
});

test('utils/common applyFormattingToTabularData does not mutate array without temporal column', () => {
  const data: TabularDataRow[] = [
    { column1: 'lorem', column2: 'ipsum' },
    { column1: 'dolor', column2: 'sit', column3: 'amet' },
  ];
...
test('utils/common applyFormattingToTabularData changes formatting of columns selected for formatting', () => {
