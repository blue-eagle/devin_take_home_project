...
test('testUtils testWithId with prefix only', () => {
  expect(testWithId('prefix')()).toEqual({ 'data-test': 'prefix' });
});

test('testUtils testWithId with prefix only and idOnly', () => {
  expect(testWithId('prefix', true)()).toEqual('prefix');
});

test('testUtils testWithId with id only', () => {
  expect(testWithId()('id')).toEqual({ 'data-test': 'id' });
});

test('testUtils testWithId with id only and idOnly', () => {
  expect(testWithId(undefined, true)('id')).toEqual('id');
});

test('testUtils testWithId with prefix and id', () => {
  expect(testWithId('prefix')('id')).toEqual({ 'data-test': 'prefix__id' });
});

test('testUtils testWithId with prefix and id and idOnly', () => {
  expect(testWithId('prefix', true)('id')).toEqual('prefix__id');
});

test('testUtils testWithId without prefix and id', () => {
  expect(testWithId()()).toEqual({ 'data-test': '' });
});

test('testUtils testWithId without prefix and id and idOnly', () => {
