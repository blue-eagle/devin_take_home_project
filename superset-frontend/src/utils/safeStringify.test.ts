...
test('Stringify utility testing correctly parses a simple object just like JSON', () => {
  const noncircular = {
    b: 'foo',
    c: 'bar',
    d: [
...
test('Stringify utility testing handles simple circular json as expected', () => {
...
test('Stringify utility testing creates a parseable object even when the input is circular', () => {
  const ping = new Noise();
  const pong = new Noise();
  ping.next = pong;
  pong.next = ping;
...
test('Stringify utility testing does not remove noncircular duplicates', () => {
  const a = {
    foo: 'bar',
  };
...
test('Stringify utility testing does not remove nodes with empty objects', () => {
...
test('Stringify utility testing does not remove nested same keys', () => {
  const nestedKeys = {
    a: 'b',
    c: {
      a: 'd',
