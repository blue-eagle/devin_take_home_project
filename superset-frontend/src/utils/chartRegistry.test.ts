...
test('Chart Registry Utils Type filter option generation generates correct options from chart metadata registry', () => {
  ...
  expect(options.find(opt => opt.value === 'native_filter')).toBeUndefined();
});

test('Chart Registry Utils Type filter option generation handles empty registry gracefully', () => {
  const options = registry
    .keys()
    .filter(k => nativeFilterGate(registry.get(k)?.behaviors || []))
    .map(k => ({ label: registry.get(k)?.name || k, value: k }));
...
test('Chart Registry Utils Type filter option generation falls back to chart key when name is missing', () => {
  ...
  expect(options).toEqual([{ label: 'custom_chart', value: 'custom_chart' }]);
});

test('Chart Registry Utils Type filter option generation sorts options alphabetically by label', () => {
  registry
    .registerValue(
      'zebra',
      new ChartMetadata({
...
test('Chart Registry Utils Type filter option generation handles mixed chart behaviors correctly', () => {
  registry
    .registerValue(
      'regular',
      new ChartMetadata({
