...
test('SQLEditorWithValidation renders SQLEditor with validation bar when showValidation is true', () => {
...
test('SQLEditorWithValidation does not render validation bar when showValidation is false', () => {
  render(<SQLEditorWithValidation {...defaultProps} showValidation={false} />);
...
test('SQLEditorWithValidation shows primary button style when unverified', () => {
  render(<SQLEditorWithValidation {...defaultProps} />);
...
test('SQLEditorWithValidation disables validate button when no value or datasourceId', () => {
  render(
...
test('SQLEditorWithValidation shows validating state when validation is in progress', async () => {
...
test('SQLEditorWithValidation shows success state when validation passes', async () => {
  const mockPost = SupersetClient.post as jest.MockedFunction<
    typeof SupersetClient.post
  >;
...
test('SQLEditorWithValidation shows error state when validation fails', async () => {
  const mockPost = SupersetClient.post as jest.MockedFunction<
    typeof SupersetClient.post
  >;
...
test('SQLEditorWithValidation handles API errors gracefully', async () => {
  const mockPost = SupersetClient.post as jest.MockedFunction<
    typeof SupersetClient.post
  >;
...
test('SQLEditorWithValidation sends correct payload for column expression', async () => {
  const mockPost = SupersetClient.post as jest.MockedFunction<
    typeof SupersetClient.post
  >;
  mockPost.mockResolvedValue({ json: { result: [] } } as any);
...
test('SQLEditorWithValidation sends correct payload for WHERE expression', async () => {
  const mockPost = SupersetClient.post as jest.MockedFunction<
    typeof SupersetClient.post
  >;
  mockPost.mockResolvedValue({ json: { result: [] } } as any);
...
test('SQLEditorWithValidation sends correct payload for HAVING expression', async () => {
  const mockPost = SupersetClient.post as jest.MockedFunction<
    typeof SupersetClient.post
  >;
  mockPost.mockResolvedValue({ json: { result: [] } } as any);
...
test('SQLEditorWithValidation resets validation state when value changes', () => {
  ...
  // Change the value
  rerender(
    <SQLEditorWithValidation {...defaultProps} value="SELECT * FROM orders" />,
  );
...
test('SQLEditorWithValidation calls onChange when editor value changes', () => {
...
test('SQLEditorWithValidation calls onValidationComplete callback when provided', async () => {
...
test('SQLEditorWithValidation calls onValidationComplete with errors when validation fails', async () => {
  const onValidationComplete = jest.fn();
  const mockPost = SupersetClient.post as jest.MockedFunction<
    typeof SupersetClient.post
  >;
  ...
  await waitFor(() => {
    expect(onValidationComplete).toHaveBeenCalledWith(false, [validationError]);
  });
});

test('SQLEditorWithValidation shows tooltip with full error message when error is truncated', async () => {
...
test('SQLEditorWithValidation handles empty response gracefully', async () => {
  const mockPost = SupersetClient.post as jest.MockedFunction<
    typeof SupersetClient.post
  >;
