...
test('chart actions should drop stale success dispatches when a newer controller has replaced ours in state', async () => {
...
test('chart actions should defer abort of previous controller to avoid Redux state mutation', async () => {
  jest.useFakeTimers();
...
test('chart actions v1 API should query with the built query', async () => {
...
test('chart actions v1 API should handle the bigint without regression', async () => {
  getChartDataUriStub.mockRestore();
...
test('chart actions v1 API handleChartDataResponse should return result if GlobalAsyncQueries flag is disabled', async () => {
  const result = await handleChartDataResponse({ status: 200 } as Response, {
    result: [
      1, 2, 3,
...
test('chart actions v1 API handleChartDataResponse should handle responses when GlobalAsyncQueries flag is enabled and results are returned synchronously', async () => {
  (
    global as unknown as { featureFlags: Record<string, boolean> }
  ).featureFlags = {
...
test('chart actions v1 API handleChartDataResponse should handle responses when GlobalAsyncQueries flag is enabled and query is running asynchronously', async () => {
  (
    global as unknown as { featureFlags: Record<string, boolean> }
  ).featureFlags = {
    [FeatureFlag.GlobalAsyncQueries]: true,
  };
  const result = await handleChartDataResponse({ status: 202 } as Response, {
    result: [
      1, 2, 3,
    ] as unknown as actions.ChartDataRequestResponse['json']['result'],
  });
...
test('chart actions legacy API should dispatch CHART_UPDATE_STARTED action before the query', () => {
  const actionThunk = actions.postChartFormData({
    viz_type: 'table',
  } as QueryFormData);
...
test('chart actions legacy API should dispatch TRIGGER_QUERY action with the query', () => {
  const actionThunk = actions.postChartFormData({
    viz_type: 'table',
  } as QueryFormData);
  return actionThunk(
...
test('chart actions legacy API should dispatch UPDATE_QUERY_FORM_DATA action with the query', () => {
  const actionThunk = actions.postChartFormData({
    viz_type: 'table',
  } as QueryFormData);
  return actionThunk(
    ...
    expect(dispatch.mock.calls.length).toBe(5);
    expect(fetchMock.callHistory.calls(MOCK_URL)).toHaveLength(1);
    expect(dispatch.mock.calls[2][0].type).toBe(actions.UPDATE_QUERY_FORM_DATA);
  });
});

test('chart actions legacy API should dispatch logEvent async action', () => {
  const actionThunk = actions.postChartFormData({
    viz_type: 'table',
  } as QueryFormData);
  return actionThunk(
...
test('chart actions legacy API should dispatch CHART_UPDATE_SUCCEEDED action upon success', () => {
  ...
  return actionThunk(
    ...
    expect(dispatch.mock.calls.length).toBe(5);
    expect(fetchMock.callHistory.calls(MOCK_URL)).toHaveLength(1);
    expect(dispatch.mock.calls[4][0].type).toBe(actions.CHART_UPDATE_SUCCEEDED);
  });
});

test('chart actions legacy API should dispatch CHART_UPDATE_FAILED action upon query timeout', () => {
  ...
  return actionThunk(
    ...
    expect(fetchMock.callHistory.calls(MOCK_URL)).toHaveLength(1);
    expect(dispatch.mock.calls.length).toBe(5);
    expect(dispatch.mock.calls[4][0].type).toBe(actions.CHART_UPDATE_FAILED);
...
test('chart actions legacy API should dispatch CHART_UPDATE_FAILED action upon non-timeout non-abort failure', () => {
  fetchMock.removeRoute(MOCK_URL);
  fetchMock.post(
    MOCK_URL,
...
test('chart actions legacy API should dispatch CHART_UPDATE_STOPPED action upon abort', () => {
  fetchMock.removeRoute(MOCK_URL);
  fetchMock.post(
    MOCK_URL,
...
test('chart actions legacy API should handle the bigint without regression', async () => {
  getExploreUrlStub.mockRestore();
  const mockBigIntUrl = '/mock/chart/data/bigint';
  const expectedBigNumber = '9223372036854775807';
  fetchMock.post(mockBigIntUrl, `{ "value": ${expectedBigNumber} }`, {
...
test('chart actions runAnnotationQuery should dispatch annotationQueryStarted and annotationQuerySuccess on successful query', async () => {
  const annotation = {
    name: 'Holidays',
...
test('chart actions timeout should use the timeout from arguments when given', async () => {
  const postSpy = jest.spyOn(SupersetClient, 'post');
  postSpy.mockImplementation(
    () =>
      Promise.resolve({ json: { result: [] } }) as unknown as ReturnType<
...
test('chart actions timeout should use the timeout from common.conf when not passed as an argument', async () => {
  const postSpy = jest.spyOn(SupersetClient, 'post');
  postSpy.mockImplementation(
    () =>
      Promise.resolve({ json: { result: [] } }) as unknown as ReturnType<
