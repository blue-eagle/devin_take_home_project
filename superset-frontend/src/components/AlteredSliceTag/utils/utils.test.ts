...
test('formatValueHandler formats unary filter', () => {
  const filters = [
    {
      expressionType: 'SIMPLE',
      operator: 'IS NULL',
...
test('getRowsFromDiffs returns formatted rows for diffs', () => {
  const diffs = {
...
test('getRowsFromDiffs falls back to key if label is missing', () => {
  const diffs = {
