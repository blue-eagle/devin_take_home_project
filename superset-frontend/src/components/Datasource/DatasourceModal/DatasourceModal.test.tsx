...
test('DatasourceModal renders', async () => {
  expect(container).toBeDefined();
});

test('DatasourceModal renders the component', () => {
  expect(screen.getByText('Edit Dataset')).toBeInTheDocument();
});

test('DatasourceModal renders a Modal', async () => {
  expect(screen.getByRole('dialog')).toBeInTheDocument();
});

test('DatasourceModal renders a DatasourceEditor', async () => {
  expect(screen.getByTestId('datasource-editor')).toBeInTheDocument();
});

test('DatasourceModal disables the save button when the datasource is managed externally', () => {
...
test('DatasourceModal calls the onDatasourceSave function when the save button is clicked', async () => {
  cleanup();
...
test('DatasourceModal should render error dialog', async () => {
  const putSpy = jest
...
test('DatasourceModal shows sync columns checkbox when SQL changes', async () => {
  cleanup();
...
test('DatasourceModal syncs columns when checkbox is checked and submits with override_columns=true', async () => {
  const datasourceWithSQL = {
    ...mockedProps.datasource,
    sql: 'SELECT * FROM original_table',
  };
...
test('DatasourceModal does not sync columns when checkbox is unchecked and submits with override_columns=false', async () => {
  const datasourceWithSQL = {
    ...mockedProps.datasource,
    sql: 'SELECT * FROM original_table',
  };
...
test('buildExtraJsonObject returns "{}" for an item with no warning and no certification', () => {
  expect(buildExtraJsonObject({} as any)).toBe('{}');
});

test('buildExtraJsonObject drops warning_markdown when its value is null', () => {
  expect(buildExtraJsonObject({ warning_markdown: null } as any)).toBe('{}');
});

test('buildExtraJsonObject drops warning_markdown when its value is an empty string', () => {
  expect(buildExtraJsonObject({ warning_markdown: '' } as any)).toBe('{}');
});

test('buildExtraJsonObject preserves a non-empty warning_markdown verbatim', () => {
  expect(buildExtraJsonObject({ warning_markdown: '⚠ caveat' } as any)).toBe(
    '{"warning_markdown":"⚠ caveat"}',
  );
});

test('buildExtraJsonObject preserves certification and drops null warning_markdown', () => {
  expect(
  ...
  ).toBe('{"certification":{"certified_by":"data-team","details":"verified"}}');
});
