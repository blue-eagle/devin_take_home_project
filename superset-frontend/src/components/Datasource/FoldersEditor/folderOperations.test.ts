...
test('folderUtils createFolder should create a folder with correct properties', () => {
...
test('folderUtils resetToDefault should create default folders with correct structure', () => {
...
test('folderUtils filterItemsBySearch should filter items by search term', () => {
...
test('folderUtils filterItemsBySearch should return all items for empty search', () => {
  const allItems = [...mockMetrics, ...mockColumns];
...
test('folderUtils filterItemsBySearch should match by verbose_name', () => {
...
test('folderUtils filterItemsBySearch should match by expression', () => {
...
test('folderUtils filterItemsBySearch should match special characters in search term', () => {
  const metrics: Metric[] = [
    {
      uuid: 'metric-special',
      metric_name: 'count',
...
test('folderUtils isDefaultFolder should identify default folders by UUID', () => {
...
test('folderUtils validateFolders should validate folders successfully', () => {
...
test('folderUtils validateFolders should allow empty folders without names', () => {
...
test('folderUtils validateFolders should detect folders with content but no name', () => {
...
test('folderUtils validateFolders should detect duplicate folder names', () => {
...
test('folderUtils validateFolders should detect duplicate folder names case-insensitively', () => {
...
test('folderUtils ensureDefaultFolders should create default folders when none exist', () => {
...
test('folderUtils ensureDefaultFolders should preserve existing folders', () => {
...
test('folderUtils ensureDefaultFolders should preserve default folder positions when saved', () => {
  ...
  const result = ensureDefaultFolders(savedFolders, mockMetrics, mockColumns);
...
test('folderUtils ensureDefaultFolders should add unassigned items to existing default folders', () => {
  const savedFolders = [
    {
      uuid: DEFAULT_METRICS_FOLDER_UUID,
      type: FoldersEditorItemType.Folder as const,
  ...
  expect(metricsFolder?.children).toHaveLength(2);
  expect(metricsFolder?.children?.some(c => c.uuid === 'metric-2')).toBe(true);

  // column-2 was unassigned, should be appended
  expect(columnsFolder?.children).toHaveLength(2);
  expect(columnsFolder?.children?.some(c => c.uuid === 'column-2')).toBe(true);
});
