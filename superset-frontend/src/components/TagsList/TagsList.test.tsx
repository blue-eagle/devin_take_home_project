...
test('Tag type filtering should render only custom type tags (type: 1)', async () => {
  const mixedTypeTags = [
  ...
  const filteredTags = mixedTypeTags.filter(tag =>
    tag.type ? tag.type === 1 || String(tag.type) === 'TagTypes.custom' : true,
  );
...
test('Tag type filtering should show tags when type is undefined (fallback case)', async () => {
...
test('Tag type filtering should handle legacy TagTypes.custom string format', async () => {
...
test('Tag type filtering should show empty list when all tags are filtered out', async () => {
  const nonCustomTags = [
...
test('Tag type filtering should handle mixed scenarios with truncation', async () => {
