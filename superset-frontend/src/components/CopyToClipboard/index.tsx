 ...
 * under the License.
 */
import { useCallback, cloneElement, ReactElement } from 'react';
...
function CopyToClip({
  ...
  const onClick = useCallback(() => {
    ...
    if (getText) {
      getText((d: string) => {
        doCopy(Promise.resolve(d));
      });
    } else {
      doCopy(Promise.resolve(text || ''));
    }
  }, [disabled, getText, text, doCopy]);
  ...
  const renderTooltip = (cursor: string) => (
    <>
      {!hideTooltip ? (
        <Tooltip
