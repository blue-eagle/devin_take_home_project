...
test('tagToSelectOption converts a Tag object with table_name to a SelectTagsValue', () => {
  const tag = {
    id: 1,
    name: 'TagName',
    table_name: 'Table1',
...
test('loadTags constructs correct API query with custom tag filter', async () => {
  const mockTags = [
...
test('loadTags returns correctly transformed data', async () => {
  const mockTags = [
    { id: 1, name: 'analytics', type: 1 },
    { id: 2, name: 'finance', type: 1 },
  ];
...
test('loadTags handles search parameter correctly', async () => {
  fetchMock.get('glob:*/api/v1/tag/*', {
    result: [],
    count: 0,
  });
...
test('loadTags handles pagination parameters correctly', async () => {
  fetchMock.get('glob:*/api/v1/tag/*', {
    result: [],
    count: 0,
  });
...
test('loadTags always includes custom tag filter regardless of other parameters', async () => {
  fetchMock.get('glob:*/api/v1/tag/*', {
    result: [],
    count: 0,
  });
...
test('loadTags maintains correct order specification', async () => {
  fetchMock.get('glob:*/api/v1/tag/*', {
    result: [],
    count: 0,
  });
