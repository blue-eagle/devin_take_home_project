...
test('RecipientIcon should render the email icon when type is Email', () => {
...
test('RecipientIcon should render the Slack icon when type is Slack', () => {
...
test('RecipientIcon should render the Slack icon when type is SlackV2', () => {
...
test('RecipientIcon should not render any icon when type is not recognized', () => {
  render(<RecipientIcon type="unknown" />);
  const icons = screen.queryByLabelText(/.*/);
  expect(icons).not.toBeInTheDocument();
});

test('RecipientIcon All recipient types should have consistent accessibility attributes', () => {
  const types = [
