...
test('DatabaseModal Visual: New database connection renders the initial load of Step 1 correctly', async () => {
  setup();
...
test('DatabaseModal Visual: New database connection shows database options when pasting text in the select', async () => {
  setup();
...
test('DatabaseModal Visual: New database connection renders the "Basic" tab of SQL Alchemy form (step 2 of 2) correctly', async () => {
  setup();
...
test('DatabaseModal Visual: New database connection renders the unexpanded "Advanced" tab correctly', async () => {
  setup();
...
test('DatabaseModal Visual: New database connection renders the "Advanced" - SQL LAB tab correctly (unexpanded)', async () => {
  setup();
  ...
  const disableSQLLabDataPreviewQueriesCheckbox = screen.getByRole('checkbox', {
...
test('DatabaseModal Visual: New database connection renders the "Advanced" - PERFORMANCE tab correctly', async () => {
  setup();
...
test('DatabaseModal Visual: New database connection renders the "Advanced" - SECURITY tab correctly', async () => {
  setup();
...
test('DatabaseModal Visual: New database connection renders the "Advanced" - SECURITY tab correctly after selecting Allow file uploads', async () => {
  setup();
...
test('DatabaseModal Visual: New database connection renders the "Advanced" - OTHER tab correctly', async () => {
  setup();
...
test('DatabaseModal Visual: New database connection Dynamic form', async () => {
  setup();
...
test('DatabaseModal Functional: Create new database directs databases to the appropriate form (dynamic vs. SQL Alchemy)', async () => {
  setup();
...
test('DatabaseModal Functional: Create new database SQL Alchemy form flow enters step 2 of 2 when proper database is selected', async () => {
  setup();
...
test('DatabaseModal Functional: Create new database SQL Alchemy form flow runs fetchResource when "Connect" is clicked', () => {
...
test('DatabaseModal Functional: Create new database SQL Alchemy form flow step 2 component interaction properly interacts with textboxes', async () => {
  setup();
...
test('DatabaseModal Functional: Create new database SQL Alchemy form flow step 2 component interaction runs testDatabaseConnection when "TEST CONNECTION" is clicked', () => {
  /* ---------- 🐞 TODO (lyndsiWilliams): function mock is not currently working 🐞 ----------
...
test('DatabaseModal Functional: Create new database SQL Alchemy form flow SSH Tunnel Form interaction properly interacts with SSH Tunnel form textboxes for dynamic form', async () => {
  setup();
...
test('DatabaseModal Functional: Create new database SQL Alchemy form flow SSH Tunnel Form interaction properly interacts with SSH Tunnel form textboxes', async () => {
  setup();
...
test('DatabaseModal Functional: Create new database SQL Alchemy form flow SSH Tunnel Form interaction if the SSH Tunneling toggle is not true, no inputs are displayed', async () => {
  setup();
...
test('DatabaseModal Functional: Create new database SQL Alchemy form flow SSH Tunnel Form interaction If user changes the login method, the inputs change', async () => {
  setup();
...
test('DatabaseModal Functional: Create new database Dynamic form flow enters step 2 of 3 when proper database is selected', async () => {
  setup();
...
test('DatabaseModal Functional: Create new database Dynamic form flow enters form credentials and runs fetchResource when "Connect" is clicked', async () => {
  setup();
...
test('DatabaseModal Functional: Create new database Import database flow imports a file', async () => {
  setup();
...
test('DatabaseModal DatabaseModal w/ Deeplinking Engine enters step 2 of 3 when proper database is selected', async () => {
  setup({ dbEngine: 'PostgreSQL' });
  const step2of3text = await screen.findByText(/step 2 of 3/i);
  expect(step2of3text).toBeInTheDocument();
});

test('DatabaseModal DatabaseModal w/ GSheet Engine enters step 2 of 2 when proper database is selected', async () => {
...
test('DatabaseModal DatabaseModal w/ GSheet Engine renders the "Advanced" - SECURITY tab without Allow File Upload Checkbox', async () => {
  setup({ dbEngine: 'Google Sheets' });
  ...
  const impersonateLoggerUserText = screen.getByText(/impersonate logged in/i);
...
test('DatabaseModal DatabaseModal w/ GSheet Engine if the SSH Tunneling toggle is not displayed, nothing should get displayed', async () => {
  setup({ dbEngine: 'Google Sheets' });
...
test('DatabaseModal DatabaseModal w errors as objects Error displays when it is an object', async () => {
  setup({ dbEngine: 'PostgreSQL' });
  const step2of3text = await screen.findByText(/step 2 of 3/i);
...
test('DatabaseModal DatabaseModal w errors as strings Error displays when it is a string', async () => {
  setup({ dbEngine: 'PostgreSQL' });
...
test('DatabaseModal DatabaseModal w Extensions should render an extension component if one is supplied', async () => {
...
test('dbReducer it will set state to payload from extra editor', () => {
  const action: DBReducerActionType = {
...
test('dbReducer it will set state to payload from editor', () => {
  const action: DBReducerActionType = {
...
test('dbReducer it will add extra payload to existing extra data', () => {
  const action: DBReducerActionType = {
    type: ActionType.ExtraEditorChange,
    payload: { name: 'foo', json: JSON.stringify({ bar: 1 }) },
  };
...
test('dbReducer it will set state to payload from extra input change', () => {
  const action: DBReducerActionType = {
...
test('dbReducer it will set state to payload from encrypted extra input change', () => {
  const action: DBReducerActionType = {
...
test('dbReducer it will set state to payload from extra input change when checkbox', () => {
  const action: DBReducerActionType = {
    type: ActionType.ExtraInputChange,
...
test('dbReducer it will set state to payload from extra input change when schema_cache_timeout', () => {
  const action: DBReducerActionType = {
    type: ActionType.ExtraInputChange,
...
test('dbReducer it will set state to payload from extra input change when table_cache_timeout', () => {
  const action: DBReducerActionType = {
    type: ActionType.ExtraInputChange,
...
test('dbReducer it will overwrite state to payload from extra input change when table_cache_timeout', () => {
  const action: DBReducerActionType = {
    type: ActionType.ExtraInputChange,
    payload: { name: 'table_cache_timeout', value: '10' },
  };
...
test(`dbReducer it will set state to payload from extra
...
test(`dbReducer it will overwrite state to payload from extra
  input change when schemas_allowed_for_file_upload`, () => {
  const action: DBReducerActionType = {
    type: ActionType.ExtraInputChange,
    payload: { name: 'schemas_allowed_for_file_upload', value: 'bar' },
...
test('dbReducer it will set state to payload from input change', () => {
  const action: DBReducerActionType = {
...
test('dbReducer it will set state to payload from input change for checkbox', () => {
  const action: DBReducerActionType = {
    type: ActionType.InputChange,
    payload: { name: 'foo', type: 'checkbox', checked: true },
  };
...
test('dbReducer it will change state to payload from input change for checkbox', () => {
  const action: DBReducerActionType = {
    type: ActionType.InputChange,
...
test('dbReducer it will add a parameter', () => {
  const action: DBReducerActionType = {
...
test('dbReducer it will add a parameter with existing parameters', () => {
  const action: DBReducerActionType = {
    type: ActionType.ParametersChange,
...
test('dbReducer it will change a parameter with existing parameters', () => {
  const action: DBReducerActionType = {
    type: ActionType.ParametersChange,
...
test('dbReducer it will set state to payload from parametersChange with catalog', () => {
  const action: DBReducerActionType = {
    type: ActionType.ParametersChange,
...
test('dbReducer it will add a new catalog array when empty', () => {
  const action: DBReducerActionType = {
...
test('dbReducer it will add a new catalog array when one exists', () => {
  const action: DBReducerActionType = {
    type: ActionType.AddTableCatalogSheet,
  };
  const currentState = dbReducer(
...
test('dbReducer it will remove a catalog when one exists', () => {
  const action: DBReducerActionType = {
...
test('dbReducer it will add db information when one is selected', () => {
...
test('dbReducer it will add a SSH Tunnel config parameter', () => {
  const action: DBReducerActionType = {
...
test('dbReducer it will add a SSH Tunnel config parameter with existing configs', () => {
  const action: DBReducerActionType = {
    type: ActionType.ParametersSSHTunnelChange,
...
test('dbReducer it will change a SSH Tunnel config parameter with existing configs', () => {
  const action: DBReducerActionType = {
    type: ActionType.ParametersSSHTunnelChange,
...
test('dbReducer it will remove the SSH Tunnel config parameters', () => {
  const action: DBReducerActionType = {
