...
test('EncryptedField Engine-to-Field Mapping handles unmapped engines gracefully', () => {
...
test('EncryptedField Parameter Value Processing handles null/undefined parameters', () => {
  const mockDb = createMockDb('gsheets', {});
  const props = { ...defaultProps, db: mockDb, isEditMode: true };
...
test('EncryptedField Conditional Rendering Logic shows upload selector in create mode', () => {
  const props = { ...defaultProps, isEditMode: false, editNewDb: false };

  render(<EncryptedField {...props} />);

  expect(
    screen.getByText('How do you want to enter service account credentials?'),
  ).toBeInTheDocument();
  expect(screen.getByRole('combobox')).toBeInTheDocument();
  expect(screen.queryByRole('textbox')).not.toBeInTheDocument();
});

test('EncryptedField Conditional Rendering Logic shows textarea in edit mode', () => {
  const props = { ...defaultProps, isEditMode: true, editNewDb: false };

  render(<EncryptedField {...props} />);

  expect(
    screen.queryByText('How do you want to enter service account credentials?'),
  ).not.toBeInTheDocument();
  expect(screen.queryByRole('combobox')).not.toBeInTheDocument();
  expect(screen.getByRole('textbox')).toBeInTheDocument();
});

test('EncryptedField Conditional Rendering Logic shows textarea when editNewDb is true', () => {
...
test('EncryptedField Upload Option State Management defaults to upload option', () => {
...
test('EncryptedField Upload Option State Management switches to copy-paste option', () => {
  const props = { ...defaultProps, isEditMode: false };
...
test('EncryptedField Form Integration Contract initializes with empty value on mount', () => {
...
test('EncryptedField Form Integration Contract renders correctly with default props', () => {
  const props = { ...defaultProps };
...
test('EncryptedField Error Boundaries renders gracefully when database prop is missing', () => {
...
test('EncryptedField Error Boundaries renders gracefully with malformed database parameters', () => {
  const mockDb = createMockDb('gsheets', {
...
test('EncryptedField Accessibility provides proper form labels and attributes', () => {
...
test('EncryptedField Accessibility provides proper labels for upload method selection', () => {
  const props = { ...defaultProps, isEditMode: false };
