...
test('RoleListEditModal renders modal with correct title and fields', () => {
...
test('RoleListEditModal calls onHide when cancel button is clicked', () => {
  render(<RoleListEditModal {...mockProps} />);
  fireEvent.click(screen.getByTestId('modal-cancel-button'));
  expect(mockProps.onHide).toHaveBeenCalled();
});

test('RoleListEditModal disables save button when role name is empty', () => {
  render(<RoleListEditModal {...mockProps} />);
...
test('RoleListEditModal enables save button when role name is entered', () => {
  render(<RoleListEditModal {...mockProps} />);
  fireEvent.change(screen.getByTestId('role-name-input'), {
...
test('RoleListEditModal submit maps {value,label} form values to numeric ID arrays', async () => {
  ...
  await waitFor(() => {
    ...
    expect(groupArg.every((id: unknown) => typeof id === 'number')).toBe(true);

    expect(mockProps.onSave).toHaveBeenCalled();
  });
});

test('RoleListEditModal switches tabs correctly', () => {
  render(<RoleListEditModal {...mockProps} />);
...
test('RoleListEditModal fetches users with correct role relationship filter', async () => {
...
test('RoleListEditModal preserves missing IDs as numeric fallbacks on partial hydration', async () => {
  const mockGet = SupersetClient.get as jest.Mock;
...
test('RoleListEditModal does not fire fallback toast when hydration fetch fails', async () => {
...
test('RoleListEditModal fires warning toast when hydration returns zero rows but IDs were expected', async () => {
  const mockGet = SupersetClient.get as jest.Mock;
  mockGet.mockImplementation(({ endpoint }) =>
...
test('RoleListEditModal does not leak state when switching roles', async () => {
  const mockGet = SupersetClient.get as jest.Mock;
...
test('RoleListEditModal fetches permissions and groups by id for hydration', async () => {
  const mockGet = SupersetClient.get as jest.Mock;
  mockGet.mockResolvedValue({
    json: {
      count: 0,
