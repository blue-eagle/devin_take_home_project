...
test('BulkTagModal should render', () => {
  const { container } = render(<BulkTagModal {...mockedProps} />);
  expect(container).toBeInTheDocument();
});

test('BulkTagModal renders the correct title and message', () => {
...
test('BulkTagModal renders tags input field', async () => {
  render(<BulkTagModal {...mockedProps} />);
  const tagsInput = await screen.findByRole('combobox', { name: /tags/i });
  expect(tagsInput).toBeInTheDocument();
});

test('BulkTagModal calls onHide when the Cancel button is clicked', () => {
  render(<BulkTagModal {...mockedProps} />);
...
test('BulkTagModal submits the selected tags and shows success toast', async () => {
...
test('BulkTagModal handles API errors gracefully', async () => {
